...
class TestSavePriceParquet(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Import modules
        try:
            cls.helper_module = importlib.import_module("third_party.Binance.helper")
            cls.schema_module = importlib.import_module("third_party.Binance.schema")
            cls.utils_module = importlib.import_module("third_party.Binance.utils")
        except Exception as e:
            print(f"Failed to import modules: {e}")
            raise
        
        # Test directory
        cls.test_root = os.path.join(cls.project_root, "test_data_parquet")
        cls.exchange = "Binance"
        cls.symbol = "TESTUSDT"
        cls.dest_dir = f"{cls.test_root}/exchange={cls.exchange}/symbol={cls.symbol}"
    ...
    def check_schema_consistency(self, parquet_file):
        ...
        # Check that date is present (as index metadata)
        # Note: When write_index=True, the index might not appear as a field
        # but should be in the schema metadata
    ...
    def test_01_initial_write(self):
        ...
        # Write data
        result = self.helper_module.save_price_parquet(
            data, 
            self.dest_dir, 
            append=False, 
            overwrite=True,
            n_partitions=None  # Auto-calculate
        )
    ...
    def test_02_append_data(self):
        ...
        self.helper_module.save_price_parquet(
            data1, 
            self.dest_dir, 
            append=False, 
            overwrite=True,
    ...
    def test_03_multiple_appends(self):
        ...
        self.helper_module.save_price_parquet(
            data1, self.dest_dir, append=False, overwrite=True
        )
        ...
        for i in range(3):
            ...
            self.helper_module.save_price_parquet(
                data, self.dest_dir, append=True
            )
            expected_rows += 50
        ...
        for pf in parquet_files:
            ...
            field_names = [field.name for field in schema]
            schemas.append(set(field_names))
        
        # All schemas should be identical
        first_schema = schemas[0]
        for schema in schemas[1:]:
            self.assertEqual(first_schema, schema, "All parquet files should have identical schema")
    ...
    def test_04_overwrite_existing(self):
        ...
        self.helper_module.save_price_parquet(
            data2, self.dest_dir, append=False, overwrite=True
        )
    ...
    def test_05_partition_isolation(self):
        ...
        self.helper_module.save_price_parquet(
            data1, dest_dir1, append=False, overwrite=True
        )
        ...
        self.helper_module.save_price_parquet(
            data2, dest_dir2, append=False, overwrite=True
        )
        ...
        self.helper_module.save_price_parquet(
            data1_new, dest_dir1, append=False, overwrite=True
        )
